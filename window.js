...
class WindowManager {
    ...
    //     //we will not resize windows if the opened window is on full screen
    //     let openedWindowActor = openedWindow.get_compositor_private()
    //     let frame = {
    //         x: openedWindow.get_frame_rect().x,
    //         y: openedWindow.get_frame_rect().y,
    //         width: openedWindow.get_frame_rect().width,
    //         height: openedWindow.get_frame_rect().height
    //     }
    //     let area = this.getUsableScreenArea(this.getCurrentMonitor())
    //     if (area)
    //         global.log("area is ", area)
    //     global.log("frame is ", frame)
    //     this.arrange()
    //     try {
    //         let windows = this.getVisibleWindowsOnCurrentMonitorBySize();
    //         let windowsToRepaint = []
    ...
    screenAppear(openedWindow) {
        ...
        // try {
        // DEBUG.windowPrint("window opened is ", openedWindow)
        // let windowDetails = {
        //     type: openedWindow.get_window_type(),
        //     appId: openedWindow.get_gtk_application_id(),
        //     id: openedWindow.get_id(),
        //     description: openedWindow.get_description(),
        // };
        // global.log("opeened window is ", JSON.stringify(windowDetails));
        // openedWindow.set_unmaximize_flags(Meta.MaximizeFlags.BOTH)
        // openedWindow.unmaximize(Meta.MaximizeFlags.BOTH)
        // openedWindow.minimize(
        // global.log("openedWindow is ", openedWindow)
        // openedWindowActor.freeze()
        // openedWindowActor.thaw()
        // global.log("demands attention", openedWindow.demands_attention)
        // global.log("effect", openedWindow.effect)
        // global.log("allows resize??", openedWindow.allows_resize())
        // global.log("allows move??", openedWindow.allows_move())
        // global.log("can close??", openedWindow.can_close())
        // global.log("can minimize??", openedWindow.can_minimize())
        // global.log("can maximize??", openedWindow.can_maximize())
        // global.log("allows resize??", openedWindow.get_maximize_flags())
        // global.log("is above??", openedWindow.is_above())
        // global.log("is fullscreen??", openedWindow.is_fullscreen())
        // global.log("allows resize??", openedWindow.is_maximized())
        // global.log("is monitor sized??", openedWindow.is_monitor_sized())
        // global.log("is screen sized", openedWindow.is_screen_sized())
        // global.log("resizeable", openedWindow.resizeable)
        // global.log("minimized", openedWindow.minimized)
        // global.log("maximized horizontally", openedWindow.maximized_horizontally)
        // global.log("maximized vertically", openedWindow.maximized_vertically)
        // global.log("fullscreen", openedWindow.fullscreen)
        // global.log("demands attention is", openedWindow.demands_attention)
        // global.log("frame type is ", openedWindow.get_frame_type())
        // global.log("maximize flags are ", openedWindow.get_maximized_flags())
        // global.log("mutter hints are ", openedWindow.get_mutter_hints())
        // global.log("transient for is ", openedWindow.get_transient_for()) 
        // global.log("has attached dialogs is ", openedWindow.has_attached_dialogs())
        // global.log("is skip taskbar is", openedWindow.is_skip_taskbar())

        // let tweenRes = Tween.isTweening(openedWindowActor)
        // global.log("tween res is ", tweenRes)
        // openedWindow.lower_with_transients(global.get_current_time()) //@works but is not the solution we are looking for
        // global.log("completed? ", global.windowManager.completed_size_change(global.WindowManager, openedWindowActor)) 
        // global.log(2, global.WindowManager)
        // global.log(3, global.window_manager)
        // global.log(4, global.windowmanager, 5, global.window_Manager)
        // openedWindow.begin_grab_op(Meta.GrabOep.WINDOW_BASE, null, global.get_current_time()) //!!SOLUTION FOUND
        //!!SOLUTION FOUND
        // global.log("global stage is ", (global.stage) instanceof Clutter.Stage) //true
        // global.log("actor grabbing is null ? ", !(global.stage.get_grab_actor()))
        // global.log("is grabed? ", !(global.stage.is_grabbed))
        // global.log("global grab is null??", !(global.grab)) //true
        // setTimeout(() => {
        //         global.log("hello advaita")
        // global.unset_cursor()
        // global.display.end_grab_op(global.get_current_time());
        // global.log("unset CURSOR")
        // let sprite = Meta.CursorTracker.get_sprite()
        // windows[0].get_meta_window().activate(global.get_current_time())
        // openedWindow.begin_grab_op(Meta.GrabOp.NONE, sprite, global.get_current_time()) //!!Solution not working but close
        //     global.log("completed? ", global.windowManager.completed_size_change(openedWindowActor)) //@works but not a solution
        //     Clutter.Grab.dismiss()
        // }, 1000)
        // global.display.clear_mouse_mode();
        // openedWindow.begin_grab_op(Meta.GrabOp.WINDOW_BASE, false, global.get_current_time()) //!!SOLUTION FOUND
        // setTimeout(() => {
        //         let sprite = Meta.CursorTracker.get_sprite()
        //         openedWindow.begin_grab_op(Meta.GrabOp.NONE, sprite, global.get_current_time())
        //     }, 1000)
        //@XDND HANDLER IN UI
        //@windowMenu in ui
        //@PLUGIN SIZE CHANGE COMPLETED
        //@cLUTTER.STAGE
        //@CLUTTER.INPUTFOCUS.RESET
        // windowDetails = {
        //     type: openedWindow.get_window_type(),
        //     appId: openedWindow.get_gtk_application_id(),
        //     id: openedWindow.get_id(),
        ...
        // global.log("demands attention is", openedWindow.demands_attention)
        // global.log("frame type is ", openedWindow.get_frame_type())
        // openedWindow.begin_grab_op(Meta.GrabOp.NONE, null, global.get_current_time()) //DOESN'T NEUTRALIZE THE EFFECT
        // openedWindow.begin_grab_op(Meta.GrapOp.NONE, null, global.get_current_time())

        // openedWindow.set_maximize_flags(null)
        //@method 1: see after a set time is window size changing if yes then arrange 
        //@method 2: see the stack view of window - not much reliable
        ...
        // global.log("Opened window frame is ", openedWindow.get_frame_rect().x, openedWindow.get_frame_rect().y, openedWindow.get_frame_rect().width, openedWindow.get_frame_rect().height)
        // }
        // catch (e) {
        // global.log("error in opened window props", e.message)
        // }
        //we will not resize windows if the opened window is on full screen
